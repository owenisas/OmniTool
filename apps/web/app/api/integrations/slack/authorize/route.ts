...
export async function GET(request: Request) {
  ...
  const slack = providerRegistry.get("SLACK");
  if (!slack) {
    return NextResponse.json(
  ...
  // Slack V2 OAuth uses `scope` for bot token scopes (not `user_scope`).
  ...
  const authUrl = `${slack.authUrl}?${params.toString()}`;
  ...
  response.cookies.set("slack-oauth-state", state, {
    httpOnly: true,
    sameSite: "lax",
    path: "/",
